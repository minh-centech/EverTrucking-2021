...
namespace cenCommonUIapps.cenLogistics.Forms
{
    public partial class frm_ctTamUngUpdate : frmBaseChungTuMasterDetail
    {
        ...
        {
            if (!coreCommon.coreCommon.IsNull(ugChiTiet.ActiveRow.Cells["IDDanhMucNguoiSuDungCreate"].Value) && ugChiTiet.ActiveRow.Cells["IDDanhMucNguoiSuDungCreate"].Value.ToString() != coreCommon.GlobalVariables.IDDanhMucNguoiSuDung.ToString())
            {
                coreCommon.coreCommon.ErrorMessageOkOnly("Bạn không có quyền sửa tạm ứng của người khác!");
                e.Cancel = true;
                return;
            }
        }
        ...
        protected override void luuChungTu(bool Exit)
        {
            ...
            bus = new ctDonHangBUS();
            Saved = (UpdateMode == coreCommon.ThaoTacDuLieu.Them || UpdateMode == coreCommon.ThaoTacDuLieu.Copy) ? bus.Insert(ref obj) : bus.Update(ref obj, false);
        ...
        protected override void xoaChungTuChiTiet()
        {
            ...
            if (!coreCommon.coreCommon.IsNull(ugChiTiet.ActiveRow.Cells["IDDanhMucNguoiSuDungCreate"].Value) && ugChiTiet.ActiveRow.Cells["IDDanhMucNguoiSuDungCreate"].Value.ToString() != coreCommon.GlobalVariables.IDDanhMucNguoiSuDung.ToString())
            {
                coreCommon.coreCommon.ErrorMessageOkOnly("Bạn không có quyền xóa tạm ứng của người khác!");
                return;
            }

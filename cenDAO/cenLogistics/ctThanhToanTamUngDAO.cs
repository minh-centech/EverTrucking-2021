    ...
    public class ctThanhToanTamUngDAO
    {
        public DataSet List(object IDDanhMucChungTu, object IDChungTu, object IDChungTuChiTiet)
        {
            ...
            if (ds != null)
            {
                ...
                ds.Relations.Add(coreCommon.GlobalVariables.prefix_DataRelation + ctThanhToanTamUng.tableName, ds.Tables[ctDonHang.tableName].Columns["ID"], ds.Tables[ctThanhToanTamUng.tableName].Columns["IDChungTu"]);
                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctThanhToanTamUng.insertProcedureName, sqlConnection, sqlTransaction))
                        {
                            ...
                            foreach (ctThanhToanTamUng obj in objList)
                            {
                                ...
                                if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, obj.ID, null, coreCommon.ThaoTacDuLieu.Them, coreCommon.coreCommon.TraTuDien(ctThanhToanTamUng.tableName), sqlConnection, sqlTransaction))
                                {
                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        foreach (ctThanhToanTamUng obj in objList)
                        {
                            if (obj.DataRowState == DataRowState.Deleted)
                            {
                                ...
                                sqlCommand.Parameters.AddRange(sqlParameters);
                                int rowAffected = sqlCommand.ExecuteNonQuery();
                                if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, null, null, coreCommon.ThaoTacDuLieu.Xoa, coreCommon.coreCommon.TraTuDien(ctThanhToanTamUng.tableName), sqlConnection, sqlTransaction))
                                {
                                    sqlTransaction.Rollback();
                                    return false;
                                }
                            ...
                            if (obj.DataRowState == DataRowState.Modified)
                            {
                                ...
                                obj.ID = Int64.Parse(sqlParameters[0].Value.ToString());
                                obj.CreateDate = DateTime.Parse(sqlParameters[sqlParameters.Length - 1].Value.ToString());
                                if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, obj.ID, null, coreCommon.ThaoTacDuLieu.Sua, coreCommon.coreCommon.TraTuDien(ctThanhToanTamUng.tableName), sqlConnection, sqlTransaction))
                                {
                                    sqlTransaction.Rollback();
                                    return false;
                                }

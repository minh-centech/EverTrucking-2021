    ...
    public class ctDonHangDAO
    {
        public DataSet List(object IDDanhMucChungTu, object ID)
        {
            ...
            if (ds != null)
            {
                ...
                ds.Relations.Add(coreCommon.GlobalVariables.prefix_DataRelation + ctDonHang.tableNameChiTiet, ds.Tables[ctDonHang.tableName].Columns["ID"], ds.Tables[ctDonHang.tableNameChiTiet].Columns["IDChungTu"]);
        ...
        public bool Insert(ref ctDonHang obj)
        {
            ...
            try
            {
                ...
                sqlParameters[38] = new SqlParameter("@IDDanhMucNguoiSuDungCreate", coreCommon.GlobalVariables.IDDanhMucNguoiSuDung);
                command.Parameters.Clear();
                command.Parameters.AddRange(sqlParameters);
                int rowAffected = command.ExecuteNonQuery();
                obj.ID = Int64.Parse(sqlParameters[0].Value.ToString());
                obj.So = sqlParameters[4].Value.ToString();
                if (NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, obj.ID, null, coreCommon.ThaoTacDuLieu.Them, coreCommon.coreCommon.TraTuDien(ctDonHang.tableName), connection, transaction))
                {
                    foreach (ctDonHang.ctDonHangChiTietTamUng ctChiTiet in obj.listChiTiet)
                    {
                        ...
                        sqlParameters[13] = new SqlParameter("@IDDanhMucNguoiSuDungCreate", coreCommon.GlobalVariables.IDDanhMucNguoiSuDung);
                        command.Parameters.Clear();
                        command.Parameters.AddRange(sqlParameters);
                        command.ExecuteNonQuery();
                        ctChiTiet.ID = Int64.Parse(sqlParameters[0].Value.ToString());
                        if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(ctChiTiet), ctChiTiet.ID, obj.ID, ctChiTiet.ID, coreCommon.ThaoTacDuLieu.Them, coreCommon.coreCommon.TraTuDien(ctDonHang.tableName), connection, transaction))
                        {
        ...
        public bool Update(ref ctDonHang obj, bool UpdateChungTu)
        {
            ...
            try
            {
                ...
                if (UpdateChungTu)
                {
                    ...
                    command.CommandType = CommandType.StoredProcedure;
                    sqlParameters = new SqlParameter[39];
                ...
                if (NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, obj.ID, null, coreCommon.ThaoTacDuLieu.Sua, coreCommon.coreCommon.TraTuDien(ctDonHang.tableName), connection, transaction))
                {
                    foreach (ctDonHang.ctDonHangChiTietTamUng ctChiTiet in obj.listChiTiet)
                    {
                        ...
                        if (ctChiTiet.DataRowState == DataRowState.Modified)
                        {
                            ...
                            sqlParameters[11] = new SqlParameter("@ThoiHanThanhToan", ctChiTiet.ThoiHanThanhToan);
                            sqlParameters[12] = new SqlParameter("@GhiChu", ctChiTiet.GhiChu);
                            sqlParameters[13] = new SqlParameter("@IDDanhMucNguoiSuDungEdit", coreCommon.GlobalVariables.IDDanhMucNguoiSuDung);
                            command.Parameters.Clear();
                            command.Parameters.AddRange(sqlParameters);
                            command.ExecuteNonQuery();
                            ctChiTiet.ID = Int64.Parse(sqlParameters[0].Value.ToString());
                            if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(ctChiTiet), ctChiTiet.ID, obj.ID, ctChiTiet.ID, coreCommon.ThaoTacDuLieu.Sua, coreCommon.coreCommon.TraTuDien(ctDonHang.tableName), connection, transaction))
                            {
                                transaction.Rollback();
                                return false;
                            }
                        ...
                        if (ctChiTiet.DataRowState == DataRowState.Deleted)
                        {
                            ...
                            command.Parameters.AddRange(sqlParameters);
                            command.ExecuteNonQuery();
                            if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(ctChiTiet), ctChiTiet.ID, obj.ID, ctChiTiet.ID, coreCommon.ThaoTacDuLieu.Xoa, coreCommon.coreCommon.TraTuDien(ctDonHang.tableName), connection, transaction))
                            {
                                transaction.Rollback();
                                return false;
                            }
                ...
                using (SqlConnection sqlConnection = new SqlConnection(coreCommon.GlobalVariables.ConnectionString))
                {
                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctDonHang.deleteProcedureName, sqlConnection, sqlTransaction))
                        {
                            ...
                            if (NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, obj.ID, null, coreCommon.ThaoTacDuLieu.Xoa, coreCommon.coreCommon.TraTuDien(ctDonHang.tableName), sqlConnection, sqlTransaction))
                            {

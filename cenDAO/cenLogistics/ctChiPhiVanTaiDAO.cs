                ...
                using (SqlConnection sqlConnection = new SqlConnection(coreCommon.GlobalVariables.ConnectionString))
                {
                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctChiPhiVanTai.insertProcedureName, sqlConnection, sqlTransaction))
                        {
                            ...
                            if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.IDChungTu, obj.ID, null, coreCommon.ThaoTacDuLieu.Them, coreCommon.coreCommon.TraTuDien(ctChiPhiVanTai.tableName), sqlConnection, sqlTransaction))
                            {
                ...
                using (SqlConnection sqlConnection = new SqlConnection(coreCommon.GlobalVariables.ConnectionString))
                {
                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctChiPhiVanTai.updateProcedureName, sqlConnection, sqlTransaction))
                        {
                            ...
                            sqlParameters[15] = new SqlParameter("@ThoiHanThanhToan", obj.ThoiHanThanhToan);
                            sqlParameters[16] = new SqlParameter("@GhiChu", obj.GhiChu);
                            sqlParameters[17] = new SqlParameter("@IDDanhMucNguoiSuDungEdit", coreCommon.GlobalVariables.IDDanhMucNguoiSuDung);
                            ...
                            if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.IDChungTu, obj.ID, null, coreCommon.ThaoTacDuLieu.Sua, coreCommon.coreCommon.TraTuDien(ctChiPhiVanTai.tableName), sqlConnection, sqlTransaction))
                            {
                                sqlTransaction.Rollback();
                                return false;
                            }
                ...
                using (SqlConnection sqlConnection = new SqlConnection(coreCommon.GlobalVariables.ConnectionString))
                {
                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctChiPhiVanTai.deleteProcedureName, sqlConnection, sqlTransaction))
                        {
                            ...
                            sqlCommand.Parameters.AddRange(sqlParameters);
                            int rowAffected = sqlCommand.ExecuteNonQuery();
                            if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, null, null, coreCommon.ThaoTacDuLieu.Xoa, coreCommon.coreCommon.TraTuDien(ctChiPhiVanTai.tableName), sqlConnection, sqlTransaction))
                            {
                                sqlTransaction.Rollback();
                                return false;
                            }

                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctChotChiPhiVanTaiBoSungGuiKeToan.insertProcedureName, sqlConnection, sqlTransaction))
                        {
                            ...
                            foreach (ctChotChiPhiVanTaiBoSungGuiKeToan obj in objList)
                            {
                                ...
                                if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, obj.ID, null, coreCommon.ThaoTacDuLieu.Them, coreCommon.coreCommon.TraTuDien(ctChotChiPhiVanTaiBoSungGuiKeToan.tableName), sqlConnection, sqlTransaction))
                                {
                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctChotChiPhiVanTaiBoSungGuiKeToan.deleteProcedureName, sqlConnection, sqlTransaction))
                        {
                            foreach (ctChotChiPhiVanTaiBoSungGuiKeToan obj in objList)
                            {
                                ...
                                sqlCommand.Parameters.AddRange(sqlParameters);
                                int rowAffected = sqlCommand.ExecuteNonQuery();
                                if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, null, null, coreCommon.ThaoTacDuLieu.Xoa, coreCommon.coreCommon.TraTuDien(ctChotChiPhiVanTaiBoSungGuiKeToan.tableName), sqlConnection, sqlTransaction))
                                {
                                    sqlTransaction.Rollback();
                                    return false;
                                }

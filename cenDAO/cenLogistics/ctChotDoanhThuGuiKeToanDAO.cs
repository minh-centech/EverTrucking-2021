                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctChotDoanhThuGuiKeToan.insertProcedureName, sqlConnection, sqlTransaction))
                        {
                            ...
                            foreach (ctChotDoanhThuGuiKeToan obj in objList)
                            {
                                ...
                                if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, obj.ID, null, coreCommon.ThaoTacDuLieu.Them, coreCommon.coreCommon.TraTuDien(ctChotDoanhThuGuiKeToan.tableName), sqlConnection, sqlTransaction))
                                {
                    ...
                    using (SqlTransaction sqlTransaction = sqlConnection.BeginTransaction())
                    {
                        using (SqlCommand sqlCommand = new SqlCommand(ctChotDoanhThuGuiKeToan.deleteProcedureName, sqlConnection, sqlTransaction))
                        {
                            foreach (ctChotDoanhThuGuiKeToan obj in objList)
                            {
                                ...
                                sqlCommand.Parameters.AddRange(sqlParameters);
                                int rowAffected = sqlCommand.ExecuteNonQuery();
                                if (!NhatKyDuLieuDAO.Insert(coreCommon.coreCommon.AllPropertiesAndValues(obj), obj.ID, obj.IDChungTu, null, coreCommon.ThaoTacDuLieu.Xoa, coreCommon.coreCommon.TraTuDien(ctChotDoanhThuGuiKeToan.tableName), sqlConnection, sqlTransaction))
                                {
                                    sqlTransaction.Rollback();
                                    return false;
                                }
